...
/**
 * Collect OpenClaw stats
 * @param {Object} options - Optional config
 * @param {string} options.dataDir - Custom OpenClaw data directory (default: ~/.openclaw)
 * @param {string} options.dockerContainer - Docker container name for OpenClaw (reads logs via docker logs)
 */
async function collectOpenClawStats(options = {}) {
  const dockerContainer = options.dockerContainer;
  const OPENCLAW_DIR = options.dataDir || path.join(os.homedir(), '.openclaw');
  ...
  try {
    ...
    // Try Docker logs first if container is specified
    if (dockerContainer) {
      try {
        const dockerLogs = execSync(`docker logs "${dockerContainer}" --tail 50 2>&1`, { encoding: 'utf8', timeout: 10000 });
        const lines = dockerLogs.trim().split('\n').filter(l => l.trim());
        logSource = 'docker';
        ...
        for (const line of lines) {
          ...
          else if (/\b(ok|success|ready|started|listening)\b/i.test(line)) level = 'OK';
          ...
          const category = catMatch ? catMatch[1] : 'gateway';
        ...
        // Docker logs failed, fall back to file logs
      }
    }
    
    // Fall back to file-based logs if no Docker entries
    if (entries.length === 0) {
